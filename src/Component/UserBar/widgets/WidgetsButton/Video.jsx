...
import { getyoutubeLink,getyoutubeTitle,getyoutubeUrl,getyoutubeLinkdesc } from "../../../../redux/CounterSlice";
    ...
    const youtubeTitle = title =>{
     ...
     const expendedYoutube = typedata =>{
     	...
     	if(typedata == 'expended'){
            if(youtubeUrl){	
		         let video_id, thumbnail, result;  
		         if(result = youtubeUrl.match(/youtube\.com.*(\?v=|\/embed\/)(.{11})/))
			        {
			            video_id = result.pop();
			        }
			        ...
			        if(video_id){
				 		  axios.get('https://www.googleapis.com/youtube/v3/videos?part=snippet&id='+video_id+'&key=AIzaSyB3K6aJw3ZahiDNZsDszkEYINOLAW7n9bI')
					      .then((response) => {
					        console.log(response.data.items[0].snippet.description);
					         dispatch(getyoutubeLinkdesc(response.data.items[0].snippet.description));
					      })
					      .catch((err) => {
					        console.log(err);
					      })
			...
			<section className="grid gap-2 items-center justify-items-start mt-5  ml-4">
				...
				<div className="grid mt-2 gap-1">
					...
					<input defaultValue={youtubeUrl || ''}
						...
						id="grid-last-name"
						type="text"
						placeholder="https://www.youtube.com/watch?v=123" onChange={(e) => toggleTab(e.target.value)}
					/>
				</div>
				<div className=" grid mt-3 gap-1 ">
					<h1>Video / Playlist title:</h1>
					<input defaultValue={youtubeTitledetail || ''}
						class="appearance-none block w-80 bg-gray-100  text-black border  border-gray-200 text-sm
                        rounded-full py-2 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
						id="grid-last-name"
						type="text"
						placeholder="Kitesurfing how to:" onChange={(e) => youtubeTitle(e.target.value)}
					/>
				</div>
					...
					<ul class="list-none flex gap-4 text-gray-600 items-center">
						<button>
							<li className="flex items-center gap-2">
								<input type="radio" defaultChecked name="videostyle" id="contactChoice1" onChange={(e) => expendedYoutube('compact')} />
								Compact
							</li>
						</button>
						<button>
							<li className="flex items-center gap-2">
								<input type="radio" name="videostyle" id="contactChoice2" onChange={(e) => expendedYoutube('expended')}  />
								Expended
							</li>
						</button>
					</ul>
